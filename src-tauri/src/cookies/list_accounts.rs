...
pub async fn discover_email_authuser_mapping(
    ...
    log::info!(
        "ListAccounts HTTP {}, body length={}, preview: {}",
        status.as_u16(),
        body.len(),
...
fn parse_list_accounts_response(body: &str) -> Result<Vec<AccountMapping>, String> {
    ...
    if let Some(captures) = post_message_re().captures(body) {
        if let Some(m) = captures.get(1) {
            ...
            log::info!(
                "ListAccounts postMessage payload (decoded): {}",
    ...
    for prefix in &[")]}'", ")]}'\n"] {
        if let Some(rest) = trimmed.strip_prefix(prefix) {
            ...
            if rest.starts_with('[') {
                log::info!(
                    "ListAccounts JSON (stripped prefix): {}",
                    rest.chars().take(500).collect::<String>()
                );
